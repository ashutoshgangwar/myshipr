...
/** One of the five permissions this app asks for. */
export type AppPermissionType =
  (typeof APP_PERMISSION_TYPES)[keyof typeof APP_PERMISSION_TYPES];

/** The alert copy and Android rationale held for each permission. */
interface PermissionMeta {
  title: string;
  message: string;
  androidRationale: Rationale;
}

/**
 * The answer every public function here returns.
 *
 * The four booleans are derived from `status` rather than replacing it, so a
 * caller can branch on the flag it cares about (`granted`, `blocked`) without
 * knowing the library's status strings — which is how every call site reads it.
 */
export interface PermissionResponse {
  ...
  unavailable: boolean;
  limited: boolean;
  /** Present on the notification paths only. */
  settings?: NotificationSettings;
  /** Present when the underlying call threw. */
  error?: unknown;
}

/** Options accepted by a permission request. */
export interface PermissionRequestOptions {
  /** Suppress the "open Settings" alert on a blocked permission. */
...
const buildPermissionResponse = (
  ...
  extra: Partial<PermissionResponse> = {},
): PermissionResponse => ({
  type,
  permission,
  status,
...
const getAndroidPermission = (type: AppPermissionType): Permission | null => {
  switch (type) {
    ...
    case APP_PERMISSION_TYPES.NOTIFICATIONS:
      // NOTE: this previously read `PERMISSIONS.ANDROID.POST_NOTIFICATIONS`,
      // which react-native-permissions v5 does not define — the library routes
      // POST_NOTIFICATIONS through checkNotifications()/requestNotifications()
      // instead of exposing it here. The property was therefore `undefined` at
      // runtime, and `null` is exactly equivalent for every caller (both fail
      // the `!permission` check). Behaviour is unchanged; see the migration
      // notes for the consequence on Android 13+.
      return null;
    default:
      return null;
  }
...
const getIosPermission = (type: AppPermissionType): Permission | null => {
  switch (type) {
    case APP_PERMISSION_TYPES.CAMERA:
...
const resolvePlatformPermission = (type: AppPermissionType): Permission | null => {
  if (isAndroid) {
...
const requestResolvedPermission = async (
  type: AppPermissionType,
...
export const requestMultipleAppPermissions = async (
  types: AppPermissionType[] = [],
  options: PermissionRequestOptions &
    Partial<Record<AppPermissionType, PermissionRequestOptions>> = {},
): Promise<Partial<Record<AppPermissionType, PermissionResponse>>> => {
  const uniqueTypes = [...new Set(types)];
  const results: Partial<Record<AppPermissionType, PermissionResponse>> = {};

  for (const type of uniqueTypes) {
    results[type] = await requestAppPermission(
      type,
      (options[type] as PermissionRequestOptions) || options,
    );
  }
