...
/**
 * @param {string} url the URL the OS handed us
 * @returns {{screen: string, params: object, action: string, url: string}|null}
 *   null when the URL is not ours, is malformed, or carries no token
 */
export const resolveDeepLink = url => {
  ...
  const build = action && ROUTES[action];
  if (!build) {
...
export const buildDeepLink = (action, token) =>
 ...
 *
 * @param {(link: {screen: string, params: object, action: string}) => void} onLink
 * @returns {() => void} unsubscribe
 */
export const initDeepLinks = onLink => {
