 ...
 * default does.
 */
export const PERIODS: ReadonlyArray<Option<EarningsPeriod>> = [
  {label: 'All', value: 'ALL'},
...
export const DEFAULT_PERIOD: EarningsPeriod = PERIODS[0].value;
 ...
 */
export const formatRowDate = (value: unknown): string => {
  if (!value) return '';
  const text = String(value).trim();
...
const rowDate = (shipment: EarningsShipment | null | undefined): string => {
 ...
 *
 * @param earnings the payload, or its `shipments` list
 */
export const toEarningsRows = (
  earnings: DriverEarnings | EarningsShipment[] | null | undefined,
) => {
  ...
  return list
    .map((shipment: EarningsShipment, index: number) => {
 ...
 */
const weekRange = (now: Date): string => {
 ...
 *
 * @param period one of `PERIODS`' values
 * @param now injectable for tests
 */
export const toPeriodCopy = (
  period: string | null | undefined,
  now: Date = new Date(),
): {range: string; grossLabel: string} => {
 ...
 * @param period the period the screen is showing
 * @param now injectable for tests
 */
export const toEarningsHeader = (
  earnings: DriverEarnings | null | undefined,
  period: string | null | undefined,
  now: Date = new Date(),
) => ({
 ...
 * @param period one of `PERIODS`' values
 */
export function useDriverEarnings(period: EarningsPeriod = DEFAULT_PERIOD) {
