...
/**
 * Every function here is fed straight from an API payload, where a figure may
 * arrive as a number, as a numeric string, or not at all. That is why the
 * parameters are this wide rather than `number`: narrowing them would push a
 * cast onto every call site and change nothing about what runs.
 */
type Numeric = number | string | null | undefined;

/**
 * A type predicate, not just a boolean: after `if (isNumber(v))` TypeScript
 * knows `v` is `number | string` and drops the null/undefined branches, which
 * is exactly how the callers use it before a `Number(v)`.
 */
export const isNumber = (value: Numeric): value is number | string =>
  Number.isFinite(Number(value)) && value !== null && value !== '';

/** "1,250" — grouped without Intl, which Hermes does not always carry. */
export const group = (value: number | string): string =>
  String(value).replace(/\B(?=(\d{3})+(?!\d))/g, ',');

/** 0.1 → "$0.10", 1250 → "$1,250" */
export const formatMoney = (value: Numeric): string => {
 ...
 */
export const formatMiles = (value: Numeric, unit: string = 'mi'): string => {
  if (!isNumber(value)) return '—';
...
/** 250 → "4h 10m", 45 → "45m" */
export const formatMinutes = (value: Numeric): string => {
  if (!isNumber(value)) return '—';
...
export const stopTime = (value: Numeric): string => {
  if (!value) return '';
