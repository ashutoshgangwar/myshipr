  ...
  routeData.routes[0].sections?.forEach(section => {
    section.tolls?.forEach(toll => {
      ...
      const currencyFares = allFares.filter(
        f => f?.price?.currency === detectedCurrency,
      );

      const pool = currencyFares.length > 0 ? currencyFares : allFares;

      const singleFares = pool.filter(f => {
      ...
      const best = (singleFares.length ? singleFares : pool).reduce((min, f) =>
        f.price.value < min.price.value ? f : min,
      );

      booths.push({
        system: toll.tollSystem || 'Toll Booth',
        amount: best.price.value,
        currency: best.price.currency,
        paymentMethods: best.paymentMethods || [],
      });

      total += best.price.value;
    });
  });
              ...
              {group.booths.map((booth, bi) => (
                <View key={bi} style={styles.boothRow}>
                  ...
                  <View style={styles.payIcons}>
                    {booth.paymentMethods.slice(0, 2).map((pm, pi) => (
