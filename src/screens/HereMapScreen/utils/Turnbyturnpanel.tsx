...
import type {ManeuverPanelProps, ManeuverStep} from './NextManeuverHud';

const LOOKAHEAD_METERS = 80;

const ACTION_ICON: Record<string, string | null> = {
  depart:         '🚀',
  arrive:         '🏁',
  turn:           null,
  keep:           null,
...
const ACTION_COLOR: Record<string, string> & {default: string} = {
  depart:         '#22C55E',
  arrive:         '#F59E0B',
...
export function resolveIcon(step?: ManeuverStep | null): string {
  if (!step) return '↑';
  const base = step.action ? ACTION_ICON[step.action] : undefined;
  if (base !== undefined && base !== null) return base;
  if (step.direction) return DIR_ICON[step.direction] ?? '↑';
  return '↑';
}
export function resolveColor(action?: string): string {
  return (action ? ACTION_COLOR[action] : undefined) ?? ACTION_COLOR.default;
}
          ...
          <View style={s.stepMeta}>
            ...
            {item.exit != null && (
              <View style={s.exitBadge}>
                <Text style={s.exitBadgeText}>Exit {String(item.exit)}</Text>
              </View>
            )}
          </View>
        </View>
...
const NextStepBanner = ({
  ...
  activeIdx?: number;
  /** Index of the upcoming maneuver. */
  nextIdx?: number;
  /** Live metres from the current position to the NEXT maneuver point. */
  metersToNext?: number | null;
  /** True when `metersToNext` ≤ 80 AND `nextIdx` > `activeIdx`. */
  ...
  const liveDist =
    metersToNext != null && Number.isFinite(metersToNext) && metersToNext > 0
      ...
      : null;
  const fallbackDist =
    typeof step.length === 'number' && Number.isFinite(step.length) && step.length > 0
    ? step.length
    : null;
  ...
  return (
    <TouchableOpacity activeOpacity={0.85} onPress={onExpand} style={s.banner}>
      ...
      <View style={s.bannerRight}>
        ...
        <Text style={s.bannerCounter}>{(displayIdx ?? 0) + 1}/{totalSteps}</Text>
...
export default function TurnByTurnPanel({
  ...
  const listRef   = useRef<FlatList<ManeuverStep> | null>(null);
  ...
  const handleStepPress = useCallback((idx: number) => setManualIndex(idx), []);
