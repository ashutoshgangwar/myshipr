...
/** A GPS fix as the snapping code reads it. */
export interface SnapInput {
  ...
  speed?: number;
  accuracy?: number;
  /** The route geometry to snap onto. */
  coords: LatLng[];
  /** Where the last snap landed, so progress cannot run backwards. */
  lastIndex?: number;
  /** The naive nearest-point result, returned when snapping is refused. */
  rawSnap?: SnapResult | null;
}
...
export function smallestBearingDelta(a: number, b: number): number {
...
export function directionAwareSnap({
  ...
  if (
    ...
    (speed == null || speed >= LOW_SPEED_MPS)
  ) {
...
export function reduceRouteCoords(
  coords: LatLng[],
...
export function resolveLiveSpeedMps(
  position: {speed?: number | null} | null | undefined,
): number | undefined {
