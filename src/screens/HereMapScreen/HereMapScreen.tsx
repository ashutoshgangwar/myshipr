...
import type {TripInfo} from '../../utils/here/mapHelpers';
import type {Coordinates, ErrorLike} from '../../types/common';
import type {ManeuverStep} from './utils/NextManeuverHud';
...
function formatTollTotal(tolls: {total?: number; currency?: string} | null | undefined): string {
...
function formatFare(price: number | undefined, currency?: string): string {
  if (typeof price !== 'number') return '—';
  const symbol = (currency ? CURRENCY_SYMBOL[currency] : undefined) ?? currency ?? '';
  return `${symbol}${price.toFixed(2)}`;
}

// ───────────────────────────────────────────────────────────────────────────
export default function HereMapScreen({navigation, route}: RootStackScreenProps<'HereMapScreen'>) {
  ...
  const [sourceLocation, setSourceLocation] = useState<MapLocation | null>(
  ...
  const [truckDetails, setTruckDetails] = useState<TruckDetails | null>(
  ...
  const [activeRoute, setActiveRoute] = useState<HereRoute | null>(null);
  ...
  const [navigationInfo, setNavigationInfo] = useState<TripInfo | null>(null);
  const [maneuverIndex, setManeuverIndex] = useState<number | null>(null);
  const [metersToNext, setMetersToNext] = useState<number | null>(null);
  const [liveSpeedKph, setLiveSpeedKph] = useState(0);
  const [speedLimitKph, setSpeedLimitKph] = useState<number | null>(null);
  ...
  const markerImagesRef = useRef<Record<string, unknown> | null>(null);
  ...
  const destinationRef = useRef<MapLocation | null>(null);
  const sourceRef = useRef<MapLocation | null>(null);
  const truckDetailsRef = useRef<TruckDetails | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setSdkError((e as ErrorLike).message ?? null);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleSelectMarkerShape = useCallback((shape: string) => {
   ...
   */
  const calculateTruckRoute = useCallback(
    ...
    return HereRouting.calculateTruckRoute(
      ...
      (truckDetailsRef.current as Record<string, unknown> | null) || null,
    );
  }, []);
  ...
  const previewRoute = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Route Error', (e as ErrorLike).message);
    } finally {
  ...
  const handleRouteDeviation = useCallback(
    async (event: {
      ...
      /** The fix that produced the deviation, used as the reroute origin. */
      currentLocation?: {latitude?: number; longitude?: number};
    }) => {
      const off = event?.deviationDistanceMeters;
      if (typeof off !== 'number' || !Number.isFinite(off) || off < OFF_ROUTE_THRESHOLD)
        return;
      ...
      try {
        ...
        console.warn('[HereMapScreen] reroute failed:', (e as ErrorLike).message);
      } finally {
  ...
  const handleStopNavigation = useCallback(async () => {
    try {
      ...
      console.warn('[HereMapScreen] stopNavigation failed:', (e as ErrorLike).message);
    }
  ...
  const handleShowLocation = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert('Location Error', (e as ErrorLike)?.message || 'Unable to fetch');
    } finally {
  ...
  const handleStartNavigation = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        'Navigation',
        (e as ErrorLike)?.message || 'Unable to start navigation',
      );
    } finally {
  ...
  const handleClear = useCallback(async () => {
    try {
      ...
      console.warn('[HereMapScreen] clear failed:', (e as ErrorLike).message);
    }
    setActiveRoute(null);
    setNavigationInfo(null);
    lastPreviewPairRef.current = '';
  ...
  useEffect(() => {
    ...
    const id = setInterval(async () => {
      try {
        ...
        if (active && typeof bearing === 'number' && Number.isFinite(bearing))
  ...
  useEffect(() => {
    ...
    const centerOnDriver = async () => {
      ...
      try {
        ...
        console.warn('[HereMapScreen] center on driver failed:', (e as ErrorLike).message);
      }
    };
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={[styles.topArea, {flex: 1}]}>
        ...
        <HereMapView
          ...
          onMapError={detail => setSdkError(detail.message ?? null)}
        />
        ...
        {isNavigating && (
          <NextManeuverHud
            steps={maneuvers as ManeuverStep[]}
        ...
        <MarkerRasterizer
          ...
          onReady={(imgs: Record<string, unknown>) => {
                    ...
                    <View style={styles.routeSummaryCard}>
                      <View style={styles.summaryItem}>
                        ...
                        <Text style={styles.summaryValue}>
                          {((routeSummary.length ?? 0) / 1000).toFixed(2)} km
                        </Text>
                      </View>
                      ...
                      <View style={styles.summaryItem}>
                        ...
                        <Text style={styles.summaryValue}>
                          {Math.ceil((routeSummary.duration ?? 0) / 60)} min
                        </Text>
                      </View>
                    </View>
                  ) : (
