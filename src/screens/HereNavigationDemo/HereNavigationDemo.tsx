...
/** The maneuver payload this demo renders. */
interface DemoManeuver {
  action?: string;
  direction?: string;
  instruction?: string;
...
/** The route-progress payload this demo renders. */
...
export default function HereNavigationDemo({navigation}: RootStackScreenProps<'HereNavigationDemo'>) {
  const mapRef = useRef<HereMapViewHandle | null>(null);

  const [sdkReady, setSdkReady] = useState(false);
  const [busy, setBusy] = useState<string | null>('Initialising HERE SDK…');
  ...
  const [maneuver, setManeuver] = useState<DemoManeuver | null>(null);
    ...
    (async () => {
      try {
        ...
        try {
          ...
          console.warn('[demo] initial camera setup skipped:', (mapErr as ErrorLike).message);
        }
      } catch (e) {
        if (!cancelled) {
          setError((e as ErrorLike).message);
        }
      } finally {
        if (!cancelled) {
          setBusy(null);
