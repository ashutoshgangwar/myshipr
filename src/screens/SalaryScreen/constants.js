...
export const moneyShort = value => money(value).replace(/\.00$/, '');

const derive = (month, raw) => {
...
export const SALARY_BY_MONTH = Object.keys(PAYROLL).reduce((acc, month) => {
  acc[month] = derive(month, PAYROLL[month]);
  return acc;
}, {});
