...
const StatusPill = ({
  ...
  <View
    style={[
      ...
      {
        backgroundColor:
          (status ? STATUS_COLOR[status] : undefined) ?? colors.textMuted,
      },
    ]}>
...
const BreakdownRow = ({
  ...
  label?: string;
  value?: number;
  /** Renders the row bold, for the total line. */
  emphasis?: boolean;
  /** Renders the figure in red, for deductions. */
  negative?: boolean;
}) => (
  <View style={styles.breakdownRow}>
    ...
    <AppText
      ...
      {negative ? money(-(value ?? 0)) : money(value ?? 0)}
    </AppText>
  </View>
);
