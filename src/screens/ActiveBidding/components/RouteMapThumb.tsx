...
const fitZoom = (
  ...
  const zoomFor = (span: number, px: number): number =>
...
const Pin = ({
  ...
  color: string;
  svgRef?: React.Ref<SvgSnapshotHandle>;
}) => (
  <Svg ref={svgRef as unknown as React.Ref<Svg>} width={RASTER_PX} height={RASTER_PX} viewBox="0 0 32 32">
    <Path
 ...
 */
export interface RouteMapThumbProps {
  pickup?: LatLng | null;
  drop?: LatLng | null;
  /** Fired when a drag starts, so a parent list can stop scrolling. */
...
export default function RouteMapThumb({
  ...
  const pickupPinRef = useRef<SvgSnapshotHandle | null>(null);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      ([['pickup', pickupPinRef]] as const).forEach(([key, ref]) => {
