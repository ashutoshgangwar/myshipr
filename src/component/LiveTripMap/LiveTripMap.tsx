...
import type {TripSession} from '../../services/TripSessionService';
...
export interface LiveTripMapProps {
  /** The live session (see TripSessionService). */
  trip?: TripSession | null;
  /** Style for the map surface. */
  style?: StyleProp<ViewStyle>;
  /** Tapping the card returns to the trip screen. */
  onExpand?: () => void;
}

/** The turn the navigator is currently calling out. */
interface Maneuver {
  action?: string;
  direction?: string;
...
export default function LiveTripMap({
  ...
  const [metersToNext, setMetersToNext] = useState<number | null | undefined>(
    null,
  );
  ...
  const bindToSession = useCallback(
    async (api?: HereMapViewHandle | null) => {
  ...
  useEffect(() => {
    const unsubscribe = HereNavigation.addListeners({
      ...
      [NavigationEvents.MANEUVER]: (next: Maneuver) => setManeuver(next),
