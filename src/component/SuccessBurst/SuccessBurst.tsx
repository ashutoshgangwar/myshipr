...
/** One confetti particle's fixed configuration. */
interface ParticleConfig {
  x: number;
  fall: number;
  size: number;
  color: string;
  /** Where in the parent's 0→1 progress this particle's life begins. */
  start: number;
  /** How much of that progress its life spans. */
  win: number;
}
...
function Particle({
  ...
  start,
  win,
}: ParticleConfig & {progress: Animated.Value}) {
  const dx = ms(x);
  const dropY = ms(fall);
  ...
  const at = (f: number): number => start + (end - start) * f;
