...
import type {MarkerOptions} from '../types/here';
...
/** Base64 PNG bytes of the rasterised pin, once one has been produced. */
let cachedImage: string | null = null;

/** The rasterised pin, or null until the first rasteriser has finished. */
export const getDestinationMarkerImage = (): string | null => cachedImage;
...
export function DestinationMarkerRasterizer() {
  const svgRef = useRef<SvgSnapshotHandle | null>(null);
  ...
  return (
    <View
      ...
      <DestinationPin
        // react-native-svg exposes toDataURL() on the mounted instance, but the
        // SVG components generated from .svg files are typed as plain FCs with
        // no ref. The cast names the handle actually available at runtime.
        ref={svgRef as unknown as React.Ref<never>}
        width={RASTER_PX}
