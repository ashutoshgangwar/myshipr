 ...
 *
 * @param {string} style one of {@link MAP_STYLE}
 * @param {boolean} night whether the night variant applies
 * @returns {string} e.g. 'normalDay' | 'normalNight' | 'hybridDay'
 */
export function resolveMapScheme(style, night = isNightNow()) {
  switch (style) {
    case MAP_STYLE.DAY:
      return 'normalDay';
...
function sanitize(stored) {
  ...
  return {
    style: styleValid ? stored.style : DEFAULT_PREFS.style,
    trafficFlow:
...
export function subscribeMapPrefs(listener) {
...
export function useMapPrefs() {
  ...
  const setStyle = useCallback(style => setMapPrefs({style}), []);
  const setTrafficFlow = useCallback(
    trafficFlow => setMapPrefs({trafficFlow}),
    [],
  );
  const setTrafficIncidents = useCallback(
    trafficIncidents => setMapPrefs({trafficIncidents}),
    [],
  );
 ...
 *
 * @param {string} [style] pass a style to pin this map to it; omit to follow
 *   the shared preference (and therefore the automatic day/night switch).
 */
export function useMapScheme(style) {
