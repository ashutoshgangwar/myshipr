...
/**
 * The native routing module. Its methods are described loosely on purpose —
 * this file's job is to forward arguments across the bridge, and each public
 * wrapper above states the real contract.
 */
type NativeRoutingModule = Record<
  string,
...
async function withSdk<T>(
  call: (mod: NativeRoutingModule) => Promise<T> | T,
): Promise<T> {
...
export function calculateCarRoute(
  ...
  return withSdk((mod: NativeRoutingModule) =>
...
export function calculateEVRoute(
  ...
  destLat: number,
  destLng: number,
  evOptions: Record<string, unknown> | null = null,
) {
  return withSdk((mod: NativeRoutingModule) =>
 ...
 */
export function releaseRoute(routeId: string) {
