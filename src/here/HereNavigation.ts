...
/** The native navigation module; each wrapper below states the real contract. */
type NativeNavigationModule = {[key: string]: (...args: any[]) => Promise<any>};
...
async function withSdk<T>(
  call: (mod: NativeNavigationModule) => Promise<T> | T,
): Promise<T> {
 ...
 *
 * @param eventName one of {@link NavigationEvents}
 * @returns call `remove()` to unsubscribe
 */
 ...
 *
 * @param handlers e.g. `{onManeuver, onRouteProgress}`
 * @returns call it to remove every subscription
 */
export function addListeners(
  ...
  const subscriptions = Object.entries(handlers)
    .filter(
      (entry): entry is [string, (payload: any) => void] =>
...
export function startNavigation(
  ...
  return withSdk((mod: NativeNavigationModule) => mod.startNavigation(routeId, options));
}
 ...
 */
export function setRoute(routeId: string) {
  if (!HereNavigationModule) {
 ...
 */
export function setCameraBehavior(camera: Record<string, unknown>) {
  if (!HereNavigationModule) {
 ...
 */
export function attachToMapView(
  mapViewTag: number | null = null,
  camera: Record<string, unknown> | null = null,
) {
 ...
 */
export function startTracking(options: Record<string, unknown> | null = null) {
  return withSdk((mod: NativeNavigationModule) => mod.startTracking(options));
}
...
export function startSimulation(
  ...
  return withSdk((mod: NativeNavigationModule) => mod.startSimulation(routeId, options));
}
