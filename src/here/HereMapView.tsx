...
/** A tap/POI event as the native component delivers it. */
type NativeMapEvent = {nativeEvent: MapTapEvent};
...
let NativeHereMapView: React.ComponentType<Record<string, unknown>> | null =
  null;
try {
 ...
 */
const HereMapView = forwardRef<HereMapViewHandle, HereMapViewProps>(
  function HereMapView(
    {
    style,
    centerLat = 0,
    centerLng = 0,
  ...
  const nativeRef = useRef<React.ComponentRef<
    NonNullable<typeof NativeHereMapView>
  > | null>(null);
  ...
  const [dataAccessWarning, setDataAccessWarning] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    HereSdk.initialize()
      .then(() => {
        ...
        return HereSdk.checkMapDataAccess().then(
          (result: {hasMapDataAccess?: boolean; message?: string}) => {
            if (cancelled || result.hasMapDataAccess) return;
            setDataAccessWarning(result.message ?? null);
  ...
  const api = useMemo<HereMapViewHandle>(() => {
    ...
    const withTag = <T,>(callback: (tag: number) => Promise<T> | T): Promise<T> => {
      ...
      return Promise.resolve(callback(tag));
    };
    ...
    return {
      ...
      loadMap: (options = null) =>
        withTag((tag: number) => HereMapModule.loadMap(tag, options)),

      /** Centres the map, without animating. */
      setCenter: (latitude: number, longitude: number, zoom = zoomLevel) =>
        withTag((tag: number) =>
          HereMapModule.setCenter(tag, latitude, longitude, zoom),
        ),

      /**
       * Shows the current-position indicator.
       * `options` is `{ bearing, style: 'navigation' | 'pedestrian' }`.
       */
      showCurrentLocation: (
        latitude?: number,
        longitude?: number,
        options: {bearing?: number; style?: string} = {},
      ) =>
        withTag((tag: number) =>
  ...
  return (
    <View style={[styles.map, style]}>
      <NativeHereMapView
        ...
        {...(onMapTap
          ? {onMapTap: (event: NativeMapEvent) => onMapTap(event.nativeEvent)}
          : {})}
        {...(onMapLongPress
          ? {
              onMapLongPress: (event: NativeMapEvent) =>
                onMapLongPress(event.nativeEvent),
            }
          : {})}
        {...(onPoiTap
          ? {onPoiTap: (event: NativeMapEvent) => onPoiTap(event.nativeEvent)}
          : {})}
        onMapError={(event: {nativeEvent: MapErrorEvent}) => {
